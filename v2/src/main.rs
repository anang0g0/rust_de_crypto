        ...
        be=p2(&be);
         for _i in 0..j {
            buf[_i]^=be[_i%32];
             ...
             //println!("{}_",a[(16 * _k + _i) % 4]^be[_i%32]);
             //buf[_i] ^=be[_i%32]; 
...
fn bfo(encoded: &String, a: &[u8; 256], mat: &Array2<u8>) -> String {
    ...
    let mut be:[u8;32]=[23;32]; //seed2.clone();
    for i in 0..3{
        be=p2(&be);
    }
    //result=pappy(result);
...
fn dec(encoded: &String, a: &[u8; 256], mat: &Array2<u8>) -> String {
    ...
    let _size: usize = 32;
    //let mut result:[u8;256]=[0;256];
    let mut nonce=[31;32]; //seed2.clone();
    for i in 0..3{
        nonce=p2(&nonce);
    }
    //result=pappy(result);
