...
let mut plain: Array2<u8> = Array2::zeros((4, 4));
let mut cipher: Array2<u8> = Array2::zeros((4, 4));
let mut tt:&str;
//let mut c:&[u8];
...
//println!("{}", data);
let v = &data[0..data.len()];
let mut bite: &[u8] = v.as_bytes(); //&[0;257]; 
let mut cs:Vec<char>;
...
for j in 0..4{
    for k in 0..4{
        //if(j*8+k <= bite.len())
        {
        plain[[k,j]]=cs[j*4+k] as u8;
        print!("{},",plain[[k,j]] as char);
        //break;
        //printf("%d,",buf[j*8+k]);
        }
        /*else{
...
println!("c=");
    for j in 0..4{
    cipher[[j,i]]=plain[[i,j]];
    print!("{},",cipher[[j,i]] as char);
    }
    println!("");
}
